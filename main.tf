# For now we have everything defined at a top-level `main.tf` file.
# At some point we will want to evolve this into separate modules
# and environments.

# GCS bucket used to store Terraform state.
resource "google_storage_bucket" "default" {
  name          = "fixie-ai-terraform-tfstate-bucket"
  force_destroy = false
  location      = "US"
...
# Deepgram On-Prem V100 instance.
resource "google_compute_instance" "deepgram-onprem" {
  boot_disk {
    auto_delete = "true"
...
# Deepgram On-Prem T4 instance.
resource "google_compute_instance" "dgonprem-t4" {
  boot_disk {
    auto_delete = "true"
    device_name = "deepgram-onprem"
    mode        = "READ_WRITE"
